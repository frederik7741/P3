...
private void CreateButtonForData(ExerciseData data) {
    ...
    // Use a local copy of 'data' to avoid the loop variable issue with closures
    ExerciseData buttonData = data;
    newButton.GetComponent<Button>().onClick.AddListener(delegate { 
        UpdatePlaceholders(buttonData); 
        InformationBox.SetActive(true);  // Assuming 'informationBox' is a GameObject that should be activated
    });
}
